                                ...
                                srfis.each { srfi ->
                                    stage("${implementation} ${srfi}") {
                                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                                            ...
                                            sh "docker run -v ${WORKSPACE}:/workdir -w /workdir -t r7rs-srfi-test-${implementation} sh -c \"make SCHEME=${implementation} SRFI=${srfi} clean package force-install test && chmod -R 755 logs && chmod -R 755 tmp\""
                                        }
                                    }
                                }
                            }
