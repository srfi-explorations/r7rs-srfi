          ...
          (for-each
            (lambda (implementation)
              (let* ((name (symbol->string (cdr (assoc 'name implementation))))
                     ...
                     (docker-image-head (if (assoc 'docker-image-head implementation)
                                       (cdr (assoc 'docker-image-head implementation))
                                       (string-append "schemers/" name ":head"))))
                (execute makefile-job
                         `((name . ,name)
                           ...
                           (docker-image-head . ,docker-image-head)
                           (number . ,number))
                         out))
              (newline out))
            implementations)))
