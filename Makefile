...
test-gambit-srfi-8:
	docker run -it -v ${PWD}:/workdir:z schemers/gambit bash -c "cd workdir &&  && gsc . srfi/8.sld"
...
test-gambit-srfi-1:
	docker run -it -v ${PWD}:/workdir:z schemers/gambit bash -c "cd workdir &&  && gsc . srfi/1.sld"
