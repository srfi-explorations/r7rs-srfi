...
test: srfi-test build index
	...
	if [ "${RNRS}" = "r6rs" ]; then ${SNOW} --install-source-dir=.tmp --install-library-dir=.tmp install srfi.64; fi
	if [ "${RNRS}" = "r6rs" ]; then ${SNOW} --install-source-dir=.tmp --install-library-dir=.tmp install srfi.${SRFI}; fi
	...
	if [ "${RNRS}" = "r7rs" ]; then ${SNOW} install srfi.64; fi
	if [ "${RNRS}" = "r7rs" ]; then ${SNOW} install srfi.${SRFI}; fi
