...
(define (inexact-number->string x) (number->string (exact x)))
(define (exact-number->string x) (number->string (inexact x)))
...
(define (format-help format-strg arglist port)
  (letrec ((length-of-format-string (string-length format-strg))
             ...
             (lambda (pos arglist last-was-newline)
               (if (>= pos length-of-format-string)
                 ...
                 (let ( (char (string-ref format-strg pos)) )
                   (cond
                     ((eqv? char #\~)
                      (tilde-dispatch (+ pos 1) arglist last-was-newline port))
                     (else
           ...
           (tilde-dispatch
             (lambda (pos arglist last-was-newline port)
               (cond
