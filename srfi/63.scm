...
#;(define array:scales
  (let ((scales (array-scales-get array:rtd)))
    (lambda (obj)
      (cond ((string? obj) '(1))
            ((vector? obj) '(1))
            (else (scales obj))))))
...
#;(define array:store
  (let ((store (array-store-get array:rtd)))
    (lambda (obj)
      (cond ((string? obj) obj)
            ((vector? obj) obj)
            (else (store obj))))))
...
#;(define array:offset
  (let ((offset (array-offset-get array:rtd)))
    (lambda (obj)
      (cond ((string? obj) 0)
            ((vector? obj) 0)
            (else (offset obj))))))
    ...
    (define (l2ra dims idxs row)
      (cond ((null? dims)
            ...
            ((if (not (eqv? (car dims) (length row)))
                 (error "list->array"
                        (list 'non-rectangular 'array dims dimensions)))
             (do ((idx 0 (+ 1 idx))
                  (row row (cdr row)))
                 ((>= idx (car dims)))
...
(define (vector->array vect prototype . dimensions)
  ...
  (if (not (eqv? vdx (apply * dimensions)))
      (error "vector->array" (list vdx '<> (cons '* dimensions))))
...
(define (make-prototype-checker name pred? creator)
  (lambda args
    (case (length args)
      ((1) (if (pred? (car args))
               (creator (car args))
               (error name (list 'incompatible 'type (car args)))))
      ((0) (creator))
      (else (error name (list 'wrong 'number 'of 'args args))))))
